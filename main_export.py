...
if __name__ == "__main__":
    ...
    while True:
        ...
        joblib.dump((observation, dict(env_cfg=configurations)), filename=export_file)

        start_time = time.time()
        o, d = joblib.load(export_file)
        actions = agent_fn(o, d)
        end_time = time.time()

        with open('data/a_monitor_execution_time.txt', 'a') as f:
            f.write(str(i) + ": " + str(end_time - start_time) + "\n")
